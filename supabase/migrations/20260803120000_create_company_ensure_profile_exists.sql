-- Fix: company_members.user_id currently FKs to public.profiles(id) on the
-- live database (schema drift from the original auth.users(id) reference —
-- the checked-in migration for company_members still shows auth.users, but
-- the live constraint was changed to profiles at some point outside of
-- version control).
--
-- create_company() only ever assumed a profiles row already existed,
-- created by the on_auth_user_created -> handle_new_user() trigger at
-- signup. That assumption breaks for any account whose profile row is
-- missing (deleted during testing, or any future signup path that
-- bypasses the trigger), causing:
--   insert or update on table "company_members" violates foreign key
--   constraint "company_members_user_id_fkey"
-- during the "Create company" step of onboarding.
--
-- Fix: defensively upsert the caller's profiles row at the top of
-- create_company(), before anything that FKs to it, so company creation
-- is self-healing regardless of whether the trigger already ran.
...
begin
  ...
  -- that FKs to it (company_members.user_id -> profiles.id). Covers cases
  -- where the on_auth_user_created trigger row was deleted or never ran.
